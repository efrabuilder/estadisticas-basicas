...
def mostrar_resultados(resultado):
    ...
    print(f"Suma:    {resultado['suma']:.4f}")
    print(f"Mínimo:  {resultado['minimo']:.4f}")
    print(f"Máximo:  {resultado['maximo']:.4f}")
    ...
    if resultado["varianza"] is None:
        ...
        print("Error típico:       n/a (se necesitan al menos 2 datos)")
    else:
        print(f"Varianza:           {resultado['varianza']:.4f}")
        print(f"Desv. estándar:     {resultado['desviacion_estandar']:.4f}")
        print(f"Error típico:       {resultado['error_tipico']:.4f}")
    ...
    print("\n--- Medidas de forma ---")
    if resultado["asimetria"] is None:
        print("Coef. de asimetría: n/a (se necesitan al menos 3 datos)")
    else:
        print(f"Coef. de asimetría: {resultado['asimetria']:.4f}")

    if resultado["curtosis"] is None:
        print("Curtosis:           n/a (se necesitan al menos 4 datos)")
    else:
        print(f"Curtosis:           {resultado['curtosis']:.4f}")
